...
output_right_video = cv2.VideoWriter(f'{video_name[:len(video_name)-5]}_left.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (frame_width, left_half_width))
output_left_video = cv2.VideoWriter(f'{video_name[:len(video_name)-5]}_right.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (frame_width, frame_height - left_half_width))
