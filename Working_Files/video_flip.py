...
def dup_video(folder_path):
    ...
    for input_video in file_path:
        if input_video.endswith('.mp4'):
            ...
            output_flip_video = cv2.VideoWriter(f'{input_video[:len(input_video)-4]}_flip.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (frame_width, frame_height))
        ...
        #an issue is with bottom v foreground naming
            while True:
                ...
                output_flip_video.write(frame)
                

            # Release resources
            cap.release()
            output_flip_video.release()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='flip a video vertically.')
